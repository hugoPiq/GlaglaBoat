...
function Archimede(objectName)
    Niv_mer = 
    z = getGPS(objectName)["altitude"]
    position, orientation = getPose(objectName)
    R = 
    g = 9.81
    V = 3.1415*
end

function forcehelice(Hdroite,Hgauche)
    coefTrac = 0.2
    coeftrai = 0.1
    Ftrac1 = rotspeedright*coefTrac
    Ftrai1 = rotspeedright*coefTrai
    force1 = {x=Ftrac1, y=0, z=0}
    Torque1 = {x=Ftrai1, y=0 ,z=0}
    sim.addForceAndTorque(Hdroite,force1,Torque1)
    Ftrac2 = rotspeedleft*coefTrac
    Ftrai2 = rotspeedleft*coefTrai
    force2 = {x=Ftrac2, y=0, z=0}
    Torque2 = {x=Ftrai2, y=0 ,z=0}
    sim.addForceAndTorque(Hgauche,force2,Torque2)
end 
function forcefrixion(ObjectHandle)
    linVel,angVel=sim.getObjectVelocity(objectHandle)
    coef_fr = 0.5
    force = {x=coef_fr*linVel[1], y=coef_fr*linVel[2], z=coef_fr*linVel[3]}
    Torque = {x=0, y=0 ,z=0}
    sim.addForceAndTorque(objectHandle,force,Torque)
end
